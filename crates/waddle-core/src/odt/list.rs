//! Lists in ODT: the flat, levelled item sequence rebuilt as nested
//! `text:list` elements, and the list styles that say which levels are
//! numbered.
//!
...
/// The `text:list-style` for a style.
pub(super) fn style_xml(style: &Style, name: &str) -> String {
...
struct Open {
    ...
    item_open: bool,
    has_items: bool,
    /// The last ordered item's number, for a break in the count.
    ...
    pub(super) fn list(&mut self, nodes: &[Node]) {
        ...
        for item in &items {
            ...
            if stack.len() == depth + 1 {
                ...
                let restart = (item.first_in_list && top.has_items)
                    || top.level.ordered != item.ordered
                    || (item.ordered && top.last_number.is_some_and(|n| item.number != n + 1));
                if restart {
                    self.close_list(&mut stack);
            ...
            if stack.len() == depth {
                let level = item.level_style();
                let mut chain: Vec<Level> = stack.iter().map(|o| o.level).collect();
                chain.push(level);
                let name = self.list_style(style(&chain, &seen));
                self.body
            ...
            top.item_open = true;
            top.has_items = true;
            top.last_number = item.ordered.then_some(item.number);
        }
...
<text:list-item><text:p text:style-name=\"List_20_Paragraph\">b</text:p>\n\
    ...
    fn a_new_list_or_a_kind_change_opens_a_new_element() {
        ...
        assert!(style_xml(&styles[0], "L1").contains("text:start-value=\"3\""));
    ...
    #[test]
    fn a_mixed_nesting_shares_one_style_whose_levels_say_which_is_which() {
        let (body, styles) = body_of(vec![
